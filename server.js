...
app.get('/api/pollution-zones', (req, res) => {
  try {
    const zonesPath = path.join(__dirname, 'pollutionZones.json');
    const zonesData = JSON.parse(fs.readFileSync(zonesPath, 'utf8'));
    res.json(zonesData);
  } catch (error) {
    console.error('❌ Error loading pollution zones:', error);
    res.status(500).json({ error: 'Failed to load pollution zones' });
  }
});
